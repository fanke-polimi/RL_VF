...
model= YOLO(model="/home/fk/opencv_3d/endo/src/endo_pos/src/yolo/last.pt")
...
class elp_stereo:
    def __init__(self):
        ...
        # self.targPOse_pub = rospy.Publisher('/elp/targ_pose', Point, queue_size=10)
    ...
    def pose_callback(self,msg):
        ...
        # point_3d = np.array([[msg.point.x, msg.point.y, msg.point.z]], dtype=np.float32)
        # rvec = np.array([-0.05073245,-0.26493352,-3.08787602], dtype=np.float32).reshape((3, 1))  # 旋转向量
        # tvec = np.array([0.036672,-0.06815654, 0.00153432], dtype=np.float32).reshape((3, 1))  # 平移向量  
        ...
        # print(self.psm_2d,type(self.psm_2d))
        # 投影3D点到图像平面
    def process_images(self):   
                ...
                ################targ detect
                # with torch.no_grad():
                results_l = model(cropped_l)
                results_r = model(cropped_r)
                ...
                #     cv2.circle(cropped_l, (int(pt_left[0]) , int(pt_left[1])), 5, (255, 255, 0), -1)
                ...
                # cv2.imshow("right", iml_rectified_l)
